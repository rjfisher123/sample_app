...
describe 'Signup confirmation' do 
	...
	describe 'visiting confirmation link' do 
		let(:bart) { FactoryGirl.create(:unconfirmed_user, name: 'Bart Simpson') }
	...
	describe 'posting before confirmation' do 
		let(:lisa) { FactoryGirl.create(:unconfirmed_user, name: 'Lisa Simpson') }
		...
		describe "should advise user to confirm email" do 
			...
			specify { expect(page).to have_content('Please confirm your email before posting') }
		end
	end
end